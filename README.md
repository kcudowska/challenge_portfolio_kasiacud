...
- dzięki nim określamy, jakie funkcjonalności należy przetestować i w jaki sposób, aby sprawdzić, czy oprogramowanie działa poprawnie oraz czy jest zgodne z założeniami biznesowymi klienta,

- służą udokumentowaniu:

  - co zostało przetestowane (jaka funkcjonalność),
  - kto testował,
  - na podstawie jakich danych,
  - z jakimi założeniami,
  - jakie wykonał kroki,
  - jakich wyników oczekiwał,

- tworzą bazę do dalszych etapów tworzenia oprogramowania, np. przy zmianach w oprogramowaniu ułatwiają wykonanie testów regresji i retestów.
...
```
UPDATE customers
SET pseudonym = concat(left(customers.name,2),right(customers.surname,1))
```
